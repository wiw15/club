  ...
  if(location.search!==""){
    ...
    $.ajax({
      ...
      success:function(result){
        ...
        .html(product.zhuti)
        .next()
        .html(`<a class="small text-dark font-weight-bold" href="javascript:;" title="${product.fuzhuti}">${product.fuzhuti}</a>`) //第二个h6
        .next() //div
        .find("div>h2") //第一个子div下的h2
        .html("¥"+product.jiage.toFixed(2))
        //第一个子div下的h2
        .parent().next() //第二个子div
        ...
        //console.log(html);
        //将多个<li>的片段放入ul中
        var $ul=$("#preview>div>div:last ul").html(html)
        //console.log($ul);
        //根据图片张数修改ul的宽
        ...
        $btnRight.click(function(){
          ...
          if($(this).is(":not(.disabled)")){
            moved++; //左移的次数+1
            //设置ul的margin-left永远等于li的宽度-62*moved
        ...
        .mousemove(function(e){
          ...
          $mask.css({ left,top });
          var backgroundPosition=`${-16/9*left}px ${-16/9*top}px`;
